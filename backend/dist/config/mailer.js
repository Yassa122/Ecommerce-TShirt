...
const transporter = nodemailer_1.default.createTransport({
    service: 'Gmail',
    auth: {
        user: process.env.EMAIL_USER || "amariaclothing.info@gmail.com",
...
const sendOrderConfirmationEmail = (email, orderDetails) => {
    const mailOptions = {
        from: 'amariaclothing.info@gmail.com',
        to: email,
      ...
      <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #f0f0f0; background-color: #333; padding: 20px; border-radius: 8px; max-width: 600px; margin: auto;">
        <div style="border: 1px solid #444; padding: 20px; border-radius: 8px; background-color: #222;">
          ...
          ${orderDetails.map((item) => `
            ...
            <hr style="border: 1px solid #555;">
          `).join('')}
          <h3 style="color: #f0f0f0;">Total: $${orderDetails.reduce((total, item) => total + item.TotalPrice, 0).toFixed(2)}</h3>
